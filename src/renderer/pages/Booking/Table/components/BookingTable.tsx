    ...
    <div className="w-full overflow-x-auto rounded-lg border border-[var(--border-color)]/20">
      <table className="w-full text-sm">
        ...
        <tbody>
          {bookings.length === 0 ? (
            <tr>
              <td colSpan={10} className="px-4 py-8 text-center text-[var(--text-tertiary)]">
                No bookings found.
              </td>
            </tr>
          ) : (
            bookings.map((booking) => (
              ...
              >
                <td className="px-4 py-3 text-[var(--text-primary)]">#{booking.id}</td>
                <td className="px-4 py-3 text-[var(--text-primary)] font-medium">
                  {booking.guest.fullName}
                </td>
                <td className="px-4 py-3">
                  <span className="text-[var(--text-primary)]">{booking.room.roomNumber}</span>
