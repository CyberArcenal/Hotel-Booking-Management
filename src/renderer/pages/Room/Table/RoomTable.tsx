...
const RoomPage: React.FC = () => {
  ...
  const handleView = (id: number) => navigate(`/rooms/${id}`);
  const handleEdit = (id: number) => navigate(`/rooms/${id}/edit`);
      ...
      {isBookingFormDialogOpen && selectedRoom && (
        <BookingFormDialog
          ...
          onSuccess={async (booking: Booking) => {
            setIsBookingFormDialogOpen(false);
            await dialogs.success(`Booking created successfully!`, `Room ${selectedRoom.roomNumber} has been booked.`);
            refetch();
          }}
        />
      )}
